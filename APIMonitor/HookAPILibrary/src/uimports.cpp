...
extern   HRESULT ReplaceOptionalSizeA(_Inout_z_count_(cchDest) LPSTR pszDest,
	_In_ size_t cchDest,
	_In_z_ LPCSTR pszSize);
extern   DWORD PadToDword(DWORD dw);
extern BOOL WINAPI  DetourVirtualProtectSameExecuteEx(_In_  HANDLE hProcess,
	_In_  PVOID pAddress,
	_In_  SIZE_T nSize,
	_In_  DWORD dwNewProtect,
	_Out_ PDWORD pdwOldProtect);
extern  DWORD PadToDwordPtr(DWORD dw);
extern PBYTE FindAndAllocateNearBase(HANDLE hProcess, PBYTE pbModule, PBYTE pbBase, DWORD cbAlloc);
                              ...
                              DWORD nDlls)
{
    ...
    if (!ReadProcessMemory(hProcess, pbModule, &idh, sizeof(idh), &cbRead)
        ...
        printf("ReadProcessMemory(idh@%p..%p) failed: %d\n",
    ...
    if (!ReadProcessMemory(hProcess, pbModule + idh.e_lfanew, &inh, sizeof(inh), &cbRead)
        || cbRead < sizeof(inh)) {
        printf("ReadProcessMemory(inh@%p..%p) failed: %d\n",
    ...
    if (inh.OptionalHeader.Magic != IMAGE_NT_OPTIONAL_HDR_MAGIC_XX) {
        printf("Wrong size image (%04x != %04x).\n",
    ...
    for (i = 0; i < inh.FileHeader.NumberOfSections; i++) {
        ...
        if (!ReadProcessMemory(hProcess, pbModule + dwSec + sizeof(ish) * i, &ish,
            ...
            printf("ReadProcessMemory(ish@%p..%p) failed: %d\n",
    ...
    if (pbNewIid == NULL) {
        printf(("FindAndAllocateNearBase failed.\n"));
        goto finish;
    }
    ...
    if (inh.IMPORT_DIRECTORY.VirtualAddress != 0) {
        ...
        printf("IMPORT_DIRECTORY perms=%x\n", dwProtect);
        ...
        if (!ReadProcessMemory(hProcess,
            ...
            printf("ReadProcessMemory(imports) failed: %d\n", GetLastError());
            goto finish;
        }
    }
    ...
    for (n = 0; n < nDlls; n++) {
        ...
        if (FAILED(hrRet)) {
            printf("StringCchCopyA failed: %d\n", GetLastError());
            goto finish;
        }
        ...
        if (FAILED(hrRet)) {
            printf("ReplaceOptionalSizeA failed: %d\n", GetLastError());
            goto finish;
        }
    ...
    for (i = 0; i < nDlls + nOldDlls; i++) {
        printf(("%8d. Look=%08x Time=%08x Fore=%08x Name=%08x Addr=%08x\n",
                      i,
    ...
    if (!DetourVirtualProtectSameExecuteEx(hProcess, pbModule, inh.OptionalHeader.SizeOfHeaders,
        ...
        printf("VirtualProtectEx(inh) write failed: %d\n", GetLastError());
        goto finish;
    }
    ...
    if (!WriteProcessMemory(hProcess, pbModule, &idh, sizeof(idh), NULL)) {
        printf("WriteProcessMemory(idh) failed: %d\n", GetLastError());
        goto finish;
    }
    printf("WriteProcessMemory(idh:%p..%p)\n", pbModule, pbModule + sizeof(idh));

    if (!WriteProcessMemory(hProcess, pbModule + idh.e_lfanew, &inh, sizeof(inh), NULL)) {
        printf("WriteProcessMemory(inh) failed: %d\n", GetLastError());
        goto finish;
    }
    printf("WriteProcessMemory(inh:%p..%p)\n",
                  pbModule + idh.e_lfanew,
                  pbModule + idh.e_lfanew + sizeof(inh));

    if (!VirtualProtectEx(hProcess, pbModule, inh.OptionalHeader.SizeOfHeaders,
                          dwProtect, &dwProtect)) {
        printf("VirtualProtectEx(idh) restore failed: %d\n", GetLastError());
        goto finish;
    }
